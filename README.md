...
- Data Reading using a customized class called ``` DataRetriever(data_path = data_path)```.The following is the command to use after downloading data to obtain a foldered data structure.
...
│   └── final_proj_data/   # Subdivided data of the desired 4 composers.
        ├── Bach/          # Subfolder containing all MIDI files of Bach
        ├── Beethoven/      # Subfolder containing all MIDI files of Beethoven
...
- **Variables and Size** `music.midi` files containing metadata information of music partitutes of 175 composers, including: Bach, Beethoven, Mozart, Brahms, Chopin, Tchaikovsky, Strauss, Stravinsky, Prokofiev, Rachmaninov, Bernstein, Bartok, Handel, Ravel, Scriabin, and others

#### Project Steps:
1. **Data Downloading/Preparation**: All data from MIDI files was downloaded, and further separated into subfolders to analyze only four desired composers: Bach, Beethoven, Chopin, and Mozart.
...
3. **EDA Piano Roll Data**: Once the piano rolls were found, EDA was done to see how the piano rolls of different durations behaved. Using different discretized times of songs, we found the median of all songs and used it as a threshold to *Equalize* all songs.
...
There are slight data imbalances that were not addressed, so a consideration to improve even more is to undersample or data augment to have a balanced dataset for training. 

<img width="1606" height="428" alt="Image" src="https://github.com/user-attachments/assets/6e70568b-2987-462b-9d3a-56521b5761a6" />

Additionally, we explored relatively small-sized compact models, so the possibility to use even deeper neural networks with more than 5 layers, or more complicated ones like computer vision transformers or autoencoders, is also a possibility.

The key thing to note was the step-by-step process to obtain an entire project from MIDI -> Piano Rolls -> Composer Labelling using `Python` and `PyTorch`.
